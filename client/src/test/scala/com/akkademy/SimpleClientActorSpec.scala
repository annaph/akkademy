...
class SimpleClientActorSpec extends FunSpecLike with Matchers {
  implicit val system = ActorSystem("akkademySimpleClient-test", ConfigFactory.empty)
  implicit val timeout = Timeout(5 seconds)

  describe("SimpleClientActor") {
    it("should set a value") {
      val dbRef = TestActorRef[AkkademyCache]
      val db = dbRef.underlyingActor
      val clientRef = TestActorRef(Props.create(classOf[SimpleClientActor], dbRef.path.toString))
      val probe = TestProbe()
